...
WORD	Pens[] = {~0};
ULONG Muster[] = { 0x01010303, 0x17170f0f, 0x1f1f3f3f, 0x7f7fffff };
...
void Render(struct RastPort *rp, WORD x, WORD y, UBYTE mask)
{
	...
	SetAfPt(rp, (UWORD *)Muster, PatSize);
...
static void __asm __saveds BackfillRoutine(
	register __a0 struct Hook *Hook,
	register __a2 struct RastPort *RPort,
	register __a1 struct LayerMsg *LayerMsg)
{
	struct RastPort	*rp = (struct RastPort *)Hook->h_Data;
	rp->BitMap = RPort->BitMap;

	RectFill(rp,
		LayerMsg->Bounds.MinX,LayerMsg->Bounds.MinY,
		LayerMsg->Bounds.MaxX,LayerMsg->Bounds.MaxY);
}

struct RastPort	HookRastPort;
struct Hook			BackFillHook = {{NULL, NULL}, (void *)BackfillRoutine, NULL, &HookRastPort};
...
main(void)
{
	...
	if(INVALID_ID != (DisplayID = p96RequestModeIDTags(TAG_DONE))){
		...
		if(sc=OpenScreenTags(NULL,
									SA_DisplayID,DisplayID,SA_Depth,4,SA_Width,640,SA_Height,480,SA_Pens,Pens,SA_FullPalette,TRUE,SA_BackFill,&BackFillHook,SA_Title,(ULONG)ScreenTitle,SA_Colors32,ColorTable,TAG_DONE)){
			...
			if(bmC = p96AllocBitMap(640,32+6*40+24,4,BMF_CLEAR|BMF_USERPRIVATE,sc->RastPort.BitMap,p96GetModeIDAttr(DisplayID,P96IDA_RGBFORMAT))){
				...
				if(bm = AllocBitMap(640,32+6*40+24,4,BMF_CLEAR,NULL)){
					...
					InitRastPort(&HookRastPort);
					SetDrMd(&HookRastPort, JAM2);
					SetAfPt(&HookRastPort, (UWORD *)Muster, PatSize);
					SetAPen(&HookRastPort, 1);
					SetBPen(&HookRastPort, 0);

					if(bdwd=OpenWindowTags(NULL,WA_Left,0,WA_Top,0,WA_Width,640,WA_Height,480,WA_CustomScreen,sc,WA_Borderless,TRUE,WA_Backdrop,TRUE,TAG_DONE)){
						CloseWindow(bdwd);
					}

					if(wd=OpenWindowTags(NULL,WA_Left,0,WA_Width,14*32,WA_Height,10*32,WA_DragBar,TRUE,WA_CloseGadget,TRUE,WA_Activate,TRUE,WA_IDCMP,IDCMP_CLOSEWINDOW|IDCMP_RAWKEY,WA_CustomScreen,sc,WA_Title,(ULONG)ScreenTitle,TAG_DONE)){
						struct Message		*msg;
