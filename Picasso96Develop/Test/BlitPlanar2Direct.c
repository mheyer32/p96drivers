...
char	ScreenTitle[] = "Picasso96 BlitPlanar2Direct Test";
WORD	Pens[] = {~0};
...
main(void)
{
	...
	if(INVALID_ID != (DisplayID = p96RequestModeIDTags(P96MA_FormatsForbidden, RGBFF_CLUT, TAG_DONE))){
		struct Screen		*sc;
		...
		if(sc=OpenScreenTags(NULL,
			...
			if(bm = AllocBitMap(4*32,13*16+8,8,BMF_CLEAR,NULL)){
				if(cbm = p96AllocBitMap(4*32,13*16+8,8,BMF_CLEAR|BMF_USERPRIVATE,sc->RastPort.BitMap,p96GetModeIDAttr(DisplayID,P96IDA_RGBFORMAT))){
					struct Window		*wd;
