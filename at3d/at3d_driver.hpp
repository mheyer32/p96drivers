...
class At3dDriver : public P96Driver
{
   public:
    ...
    void ASM blitTemplate(__REGA1(struct RenderInfo *ri), __REGA2(struct Template *tmpl), __REGD0(WORD x),
                          ...
                          __REGD7(RGBFTYPE_REG fmt));
    void ASM blitTemplate6422(__REGA1(struct RenderInfo *ri), __REGA2(struct Template *tmpl), __REGD0(WORD x),
                              __REGD1(WORD y), __REGD2(WORD width), __REGD3(WORD height), __REGD4(UBYTE mask),
                              __REGD7(RGBFTYPE_REG fmt));
    ...
    void performPlanarPlaneBlit(UWORD width, UWORD height, UBYTE *bitmap, UWORD dwordsPerLine, WORD bmPitch, UBYTE rol,
                                UBYTE planeIndex);
...
static INLINE At3dDriver *asAt3d(BoardInfo *bi)
{
    return static_cast<At3dDriver *>(bi);
}
static INLINE const At3dDriver *asAt3d(const BoardInfo *bi)
{
    return static_cast<const At3dDriver *>(bi);
}
