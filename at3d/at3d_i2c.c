...
BOOL at3dI2cInit(struct BoardInfo *bi)
{
    ...
    dpmsReg &= ~(I2C_SCL_CTRL_MASK | I2C_SDA_CTRL_MASK);
...
void at3dI2cSetScl(struct BoardInfo *bi, BOOL high, BOOL checkClockStretching)
{
    ...
    // Clear SCL control bits first
    dpmsReg &= ~I2C_SCL_CTRL_MASK;
    
    if (high) {
        // Release SCL (set to input/tri-state)
        // 0x = input/tri-state (both bits clear - already done above)
        W_MMIO_B(DPMS_SYNC_CTRL, dpmsReg);
        ...
        while (settle_attempts-- > 0) {
            ...
            if (statusReg & (1UL << I2C_SCL_IN_BIT)) {
                scl_high = TRUE;
                break;
            }
            delayMicroSeconds(I2C_DELAY_US);
        ...
        W_MMIO_B(DPMS_SYNC_CTRL, dpmsReg | I2C_SCL_CTRL_LOW);
        delayMicroSeconds(I2C_DELAY_US);
...
void at3dI2cSetSda(struct BoardInfo *bi, BOOL high)
{
    ...
    // Clear SDA control bits first
    dpmsReg &= ~I2C_SDA_CTRL_MASK;
    ...
    if (high) {
        ...
        W_MMIO_B(DPMS_SYNC_CTRL, dpmsReg | I2C_SDA_CTRL_LOW);
        delayMicroSeconds(I2C_DELAY_US);
        ...
        ULONG statusReg = R_MMIO_L(EXT_DAC_STATUS);
        if (statusReg & (1UL << I2C_SDA_IN_BIT)) {
...
BOOL at3dI2cReadScl(struct BoardInfo *bi)
{
    ...
    ULONG statusReg = R_MMIO_L(EXT_DAC_STATUS);
    return (statusReg & (1UL << I2C_SCL_IN_BIT)) != 0;
}
...
BOOL at3dI2cReadSda(struct BoardInfo *bi)
{
    ...
    ULONG statusReg = R_MMIO_L(EXT_DAC_STATUS);
    return (statusReg & (1UL << I2C_SDA_IN_BIT)) != 0;
}
