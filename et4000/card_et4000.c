...
#ifndef TESTEXE
const char LibName[]     = "ET4000.card";
const char LibIdString[] = "Tseng ET4000/W32p Picasso96 card driver version 1.0";
...
#define CHIP_NAME_ET4000 "picasso96/ET4000.chip"
...
BOOL FindCard(__REGA0(struct BoardInfo *bi))
{
    ...
    while (board = FindBoard(board, PRM_Vendor, VENDOR_ID_TSENG, TAG_END)) {
        D(INFO, "Tseng board found\n");
        ...
        D(INFO, "Tseng device %lx revision %lx\n", deviceId, revision);
        ChipFamily_t chipFamily = getChipFamily(deviceId, revision);
        if (chipFamily == ET4000_UNKNOWN) {
        ...
        if (owner) {
            ...
            if (!cd->board) {
                cd->boardNode.ln_Name = "ET4000.card";
...
BOOL InitCard(__REGA0(struct BoardInfo *bi), __REGA1(char **ToolTypes))
{
    ...
    if (!cd->board || !cd->OpenPciBase) {
        DFUNC(ERROR, "ET4000.card: No board claimed\n");
        return FALSE;
    }
    ...
    if (!initRegisterAndMemoryBases(bi)) {
        D(ERROR, "ET4000.card: could not init card\n");
        return FALSE;
    }

    struct ChipBase *ChipBase = NULL;

    if (!(ChipBase = (struct ChipBase *)OpenLibrary(CHIP_NAME_ET4000, 0))) {
        D(ERROR, "ET4000.card: could not open chip library %s\n", CHIP_NAME_ET4000);
        return FALSE;
    }
