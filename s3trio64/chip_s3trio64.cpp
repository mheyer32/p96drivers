...
#define TO_SCANLINES(y)   toScanLines((y), modeFlags)
...

    {
        ...
        vga.readB(VgaReg::INPUT_STATUS1_R);
                              ...
                              __REGD1(WORD xoffset), __REGD2(WORD yoffset), __REGD7(RGBFTYPE_REG format))
{
    ...
    vga.readB(VgaReg::INPUT_STATUS1_R);  // Reset AFF flip-flop // FIXME: why?
...
BOOL ASM S3Driver::getVSyncState(__REGD0(BOOL expected))
{
    DFUNC(VERBOSE, "\n");
    VgaIo vga = this->vga();
    return (vga.readB(VgaReg::INPUT_STATUS1_R) & 0x08) != 0;
}
...
void ASM S3Driver::waitVerticalSync(__REGD0(BOOL waitForEnd))
{
    ...
    if (waitForEnd) {
        ...
        while (!(vga.readB(VgaReg::INPUT_STATUS1_R) & 0x08)) {
        };
        // For pixel display (should now be top of frame, i.e. end of retrace)
        while (!(vga.readB(VgaReg::INPUT_STATUS1_R) & 0x01)) {
        };
...
extern "C" BOOL InitChip(__REGA0(struct BoardInfo *bi))
{
    ...
    */
    ...
    // Enable video
    vga.readB(VgaReg::INPUT_STATUS1_R);  // reset AFF
    vga.writeB(VgaReg::ATTR_AD, 0x20);
